...
if(hash!=null && hash.length>0){
	...
	BOOK_BASE_RATE = userAttInfo.getString("BOOK_BASE_RATE");
...
//                        barcode = BarcodeFactory.create3of9(num, false);
		...
		<td width="250" ><BR><img src="./barcodeImage/<%=EmsNumberUtil.format(k++, "000")%>.jpg" width="200"><div align="center" style="padding-left:5"  ><!--<P>기본료:<%=BOOK_BASE_RATE%>원<BR><font class="barcode1" >(시간당 <%=BOOK_BASE_RATE%>원 계산됩니다)</font>--><BR>과자,음료수,라면은<BR>카운터에 문의<P><B>찾아주셔서 감사합니다.<BR><%=USERNAME %></B><BR>&nbsp;</div></td>
		<td             ><BR><img src="./barcodeImage/<%=EmsNumberUtil.format(k++, "000")%>.jpg" width="200"><div align="center" style="padding-left:5"  ><!--<P>기본료:<%=BOOK_BASE_RATE%>원<BR><font class="barcode1" >(시간당 <%=BOOK_BASE_RATE%>원 계산됩니다)</font>--><BR>과자,음료수,라면은<BR>카운터에 문의<P><B>찾아주셔서 감사합니다.<BR><%=USERNAME %></B><BR>&nbsp;</div></td>
		<td             ><BR><img src="./barcodeImage/<%=EmsNumberUtil.format(k++, "000")%>.jpg" width="200"><div align="center" style="padding-left:5"  ><!--<P>기본료:<%=BOOK_BASE_RATE%>원<BR><font class="barcode1" >(시간당 <%=BOOK_BASE_RATE%>원 계산됩니다)</font>--><BR>과자,음료수,라면은<BR>카운터에 문의<P><B>찾아주셔서 감사합니다.<BR><%=USERNAME %></B><BR>&nbsp;</div></td>
		<td             ><BR><img src="./barcodeImage/<%=EmsNumberUtil.format(k++, "000")%>.jpg" width="200"><div align="center" style="padding-left:5"  ><!--<P>기본료:<%=BOOK_BASE_RATE%>원<BR><font class="barcode1" >(시간당 <%=BOOK_BASE_RATE%>원 계산됩니다)</font>--><BR>과자,음료수,라면은<BR>카운터에 문의<P><B>찾아주셔서 감사합니다.<BR><%=USERNAME %></B><BR>&nbsp;</div></td>
		</tr>
